...
static int snake_colors[NUM_SNAKES] = { 1, 6 }; 
static int game_running = 1;
static int can_eat_snakes = 1;
static int sound_enabled = 1;


static int self_collision_kills = 0; 
static int wall_collision_kills = 0; 
...
static int32_t normalizeKey(int32_t key) {
    if (key >= 0 && key <= 255) {
        return (int32_t)tolower((unsigned char)key);
    }
    return key;
}
...
static int keyToDirByControls(const snake_t *snake, int32_t key, int *out_dir) {
    int32_t k = normalizeKey(key);

    if (normalizeKey(snake->controls.up)    == k) { *out_dir = UP;    return 1; }
    if (normalizeKey(snake->controls.down)  == k) { *out_dir = DOWN;  return 1; }
    if (normalizeKey(snake->controls.left)  == k) { *out_dir = LEFT;  return 1; }
    if (normalizeKey(snake->controls.right) == k) { *out_dir = RIGHT; return 1; }

    return 0;
}


static int keyToDirRussianWASD(const snake_t *snake, int32_t key, int *out_dir) {
    
    if (!(snake->controls.up == 'w' || snake->controls.up == 'W')) return 0;

    switch(key) {
        case 1094: case 1062: *out_dir = UP;    return 1; 
        case 1099: case 1067: *out_dir = DOWN;  return 1; 
        case 1092: case 1060: *out_dir = LEFT;  return 1; 
        case 1074: case 1042: *out_dir = RIGHT; return 1; 
    }
    return 0;
}
...
int checkDirection(snake_t* snake, int32_t key) {
    ...
    if (!keyToDirByControls(snake, key, &new_dir)) {
       
        if (!keyToDirRussianWASD(snake, key, &new_dir)) {
            return 0; 
        }
    }

    
    if (isOpposite(snake->direction, new_dir)) {
        return 0;
    }
...
void changeAllDirections(snake_t snakes[], size_t num_snakes, const int32_t key) {
    for (size_t i = 0; i < num_snakes; i++) {
        if (!snakes[i].is_alive) continue;
        (void)checkDirection(&snakes[i], key);
    }
}
...
int isCrush(snake_t *snake) {
    if (!self_collision_kills) return 0;

    if (snake->tsize <= 2) return 0; 
...
void showWelcomeScreen() {
    ...
    mvprintw(7, 5, "Snake 2: WASD (case-insensitive)");
